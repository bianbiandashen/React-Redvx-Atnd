...
import { AddUserInfo, Login, LoginOut }  from  '../../action/actions';
...
//laowang code
@withRouter @connect(mapStateToProps, mapDispatchToProps) @Form.create()
class LoginForm extends React.Component {
  ...
  convertPropsToStates(props) {
    ...
    const users = userEntity && values(userEntity);
  ...
  loginSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        // 检测用户名是否存在
        const result = users && users.length> 0 && users.find(item => item.username === values.username)
        if (!result) {
          this.props.form.setFields({
            username: {
